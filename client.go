...
// bypassing dgman's additive-only CreateSchema path. Use for predicate renames,
// type drops, or any alter that UpdateSchema cannot express.
func (c client) AlterSchema(ctx context.Context, schema string) error {
	...
	return conn.Alter(ctx, &api.Operation{Schema: schema})
}
